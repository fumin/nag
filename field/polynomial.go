...
func equalDegree[K Finite[K]](f *nag.Polynomial[K], d *big.Int) []*nag.Polynomial[K] {
	...
	q := new(big.Int).Exp(k.Characteristic(), k.PrimePower(), nil)
...
// pow sets returns x^n, computed via exponentiation by squaring, so that n may be astronomically large.
func pow[K nag.Group[K]](x K, n *big.Int) K {
	switch {
	case n.Sign() < 0:
...
func mulInt[K nag.Field[K]](x K, n *big.Int) K {
	return pow(&fieldAddGroup[K]{x}, n).field
}
...
func Parse[K Finite[K]](field K, input string) (*nag.Polynomial[K], error) {
	...
	for rc, w := range rp.Terms() {
		c := field.NewZero().Div(field.Ith(rc.Num()), field.Ith(rc.Denom()))
