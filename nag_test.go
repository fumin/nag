	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			for _, gi := range test.g {
				testg = append(testg, NewPolynomial(Rationals, Deglex).Set(gi))
			}
...
func TestDivide(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			quotient := [][]Quotient[*Rat]{}
			testf := NewPolynomial(Rationals, Deglex).Set(test.f)
			...
			for i := range quotient {
				for j := range quotient[i] {
					c := NewPolynomial(Rationals, Deglex, PolynomialTerm[*Rat]{Coefficient: quotient[i][j].Coefficient})
					left := NewPolynomial(Rationals, Deglex, PolynomialTerm[*Rat]{Coefficient: NewRat(1, 1), Monomial: quotient[i][j].Left})
					right := NewPolynomial(Rationals, Deglex, PolynomialTerm[*Rat]{Coefficient: NewRat(1, 1), Monomial: quotient[i][j].Right})
...
func TestPolynomialAdd(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			// z = x.
			x := NewPolynomial(Rationals, Deglex).Set(test.x)
			z = x
			z.Add(x, test.y)
			if !z.Equal(test.z) {
				t.Errorf("%v", z)
			...
			// z = y.
			y := NewPolynomial(Rationals, Deglex).Set(test.y)
			z = y
			z.Add(test.x, y)
			if !z.Equal(test.z) {
				t.Errorf("%v", z)
...
func TestPow(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			t.Parallel()
			z := NewPolynomial(Rationals, Deglex)
			z.Pow(test.x, test.y)
			if !z.Equal(test.z) {
				t.Errorf("%v", z)
			}
...
func TestMulScalar(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			z = NewPolynomial(Rationals, Deglex).Set(test.x)
